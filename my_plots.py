...
def one_hit_wonders(ohw_data, year):
...
def top_names_by_letter_plot(df, letter, n=10, width=800, height=600, variable='count'):
    # Ensure the letter parameter is a single character string
...
def top_letter_plot(df, letter, n=10, width=800, height=600, variable='count'):
    ...
    male_names = letter_data[letter_data['sex'] == 'F']
    top_male = male_names.sort_values(variable, ascending=False).head(n)
    top_male['sex_rank'] = range(1, n + 1)  # Rank within male names
    ...
    fig.update_layout(title=f'Top {n} by sex names by letter {letter}',
                    width=width, height=height)  
    return fig  